...
def train(hyp,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if not no_val or final_epoch:  # Calculate mAP
            # TODO (Mai): validation on train_loader
            train_results, train_maps, _ = val_v2.run(data_dict,
                                          batch_size=batch_size * 2,
                                          img_size=img_size,
                                          model=ema.ema,
                                          dataloader=train_loader,
                                          save_dir=save_dir,
                                          verbose=num_class < 50,  # remove the final_epoch condition
                                          plots=plots,  # remove final_epoch condition
                                          callbacks=callbacks,
                                          compute_loss=compute_loss)
            # TODO (Mai): end
            results, maps, _ = val_v2.run(data_dict,
                                       batch_size=batch_size * 2,
                                       img_size=img_size,
                                       model=ema.ema,
